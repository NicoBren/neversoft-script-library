...
script mainmenu_item_focus 
	...
	<parent_id> :GetTags
	GetArraySize \{$Launcher_Games_Array}
	arrayIndex = (<tag_selected_index> + $game_array_offset)
	...
	if (<i> <= <num_songs>)
		...
		AddParam structure_name = text_props name = <song_text> value = qs("")
		AddParam structure_name = text_props name = <artist_text> value = qs("")
	endif
	<i> = (<i> + 1)
	repeat 4
...
script BX_update_helper_pills 
	...
	if ((($<curr_game>).ContentType) = movie)
		add_user_control_helper \{text = qs(0x8494b0f4)
			button = green
			z = 10
			all_buttons
			override_body_alpha_to = 0}
	elseif ((($<curr_game>).ContentType) = demo)
		add_user_control_helper \{text = qs("PLAY")
			button = green
			z = 10
			all_buttons
			override_body_alpha_to = 0}
...
script GameBox_Animation 
	...
	elseif (<direction> = right)
		...
		if (<GameBox_Index> < 0)
			GameBox_Index = (<GameBox_Index> + <GAAR>)
		endif
		curr_game = ($Launcher_Games_Array [<GameBox_Index>])
		curr_gamebox = (<Acc> [<gamma>])
		<curr_gamebox> :SE_SetProps {
...
script BX_video_preview 
	...
	if GotParam \{curr_game}
		...
		if current_menu_anchor :Desc_ResolveAlias \{name = alias_video_container}
			...
			printf \{qs("\LPAS DE alias_video_container")
				channel = DEMOLAUNCHER}
		endif
	endif
endscript
...
script BX_play_video 
	...
	if ((($<curr_game>).ContentType) = demo)
		add_user_control_helper \{text = qs(0x6b2616c4)
			button = green
			z = 120000
			all_buttons
			override_pos_to = (0.0, 60.0)
			override_anchor_to = [
				right
				top
			]
		...
		if NOT ScreenElementExists \{id = helper_pills_movie_background}
			CreateScreenElement \{type = DescInterface
				parent = root_window
				id = helper_pills_movie_background
				desc = 'helper_pills_background'}
		endif
	endif
...
endscript
game_array_offset = 0
g_demo_timeout = 360000

script set_start_title 
	if (<demo_timeout> >= 360000)
