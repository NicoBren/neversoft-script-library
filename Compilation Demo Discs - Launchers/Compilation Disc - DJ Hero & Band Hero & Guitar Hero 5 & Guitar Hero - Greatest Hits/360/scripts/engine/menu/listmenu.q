...
script add_list_item \{focus_script = menu_list_focus
	...
	if ScreenElementExists id = <parent>
		CreateScreenElement {
			...
			generic_list_menu_item_text_dims = <text_size>
			generic_list_menu_item_text_internal_scale = <internal_scale>
			tags = {Highlight_star_2_alpha = 0}
			pos = (50.0, 0.0)
		}
	else
...
script menu_list_focus 
	...
	Wait \{1
		gameframe}
	SE_SetProps generic_list_menu_item_text_rgba = (($g_menu_colors).menu_title)
	params = {ObjID = <ObjID>}
	if GotParam \{Highlight_star_2_alpha}
		params = {<params> Highlight_star_2_alpha = <Highlight_star_2_alpha>}
	endif
	BX_FocusHighlightStarsOn_GenericMenu <params>
endscript
...
script menu_list_unfocus 
	...
	SE_SetProps generic_list_menu_item_text_rgba = (($g_menu_colors).menu_gold)
endscript
...
script pad_choose_dialogue_execute \{pad_choose_script = nullscript
	...
	if ScreenElementExists \{id = generic_list_menu}
		generic_list_menu :SE_SetProps {
			generic_list_menu_dialogue_text_text = <pad_choose_dialogue>
			generic_list_menu_item_price_text_text = <price_text>
			generic_list_menu_dialogue_menu_yes_text = qs("Yes")
			generic_list_menu_dialogue_menu_no_text = qs("No")
			generic_list_menu_dialogue_menu_padding_scale = 3
			generic_list_menu_up_arrow_alpha = 0
			generic_list_menu_down_arrow_alpha = 0
			generic_list_menu_dialogue_pos = {relative (10.0, 0.0)}
		}
...
script pad_choose_dialogue_return 
	...
	generic_list_menu :SE_SetProps \{generic_list_menu_dialogue_pos = {
			relative
			(-10.0, 0.0)
		}}
endscript
...
script continue_purchase_item 
	...
	if has_enough_money price = <price>
		...
		generic_list_menu :SE_SetProps \{generic_list_menu_dialogue_text_text = qs("You do not have enough money to purchase this item.")
			...
			generic_list_menu_dialogue_menu_padding_scale = 2
			generic_list_menu_up_arrow_alpha = 0
			generic_list_menu_down_arrow_alpha = 0}
		...
		if generic_list_menu :Desc_ResolveAlias \{name = alias_generic_list_menu_dialogue_menu_yes
			...
			LaunchEvent target = <generic_list_dialogue_yes> type = focus
		endif
	endif
endscript
...
script BX_AnimArrow 
	if GotParam \{up}
		arrow_prop_grow = {generic_list_menu_up_arrow_scale = 0.9}
		arrow_prop_shrink = {generic_list_menu_up_arrow_scale = 0.7}
	else
		arrow_prop_grow = {generic_list_menu_down_arrow_scale = 0.9}
		arrow_prop_shrink = {generic_list_menu_down_arrow_scale = 0.7}
	endif
	SetScreenElementProps {
		id = generic_list_menu
		<arrow_prop_grow>
	...
	WaitScreenElementProps \{id = generic_list_menu}
	SetScreenElementProps {
		id = generic_list_menu
		<arrow_prop_shrink>
		time = 0.1
