...
net_game_type_info = [
	{
		name = qs("1v1 PRO FACE-OFF (Guitar)")
	...
	{
		name = qs("2v2 PRO FACE-OFF (Guitar)")
		description = qs("Play a song with the same notes.")
...
script online_fail_song 
	...
	if NOT ($is_attract_mode = 1)
		printf \{qs("\LUpdating from online_fail_song...")}
		Achievements_Update
	endif
endscript
...
script net_get_player_instrument 
	...
	if IsWinPort
		GetPlayerInfo player = (<controller_index> + 1) part
		return instrument = <controller_type> part = <part> <...>
	else
...
script transfer_controller_value_for_local_player 
	...
	if IsWinPort
		if NOT GotParam \{player_found}
			if (($temp_net_id [0].net_id_first) = <net_id_first> && ($temp_net_id [0].net_id_second) = <net_id_second>)
				change structurename = <player_status> controller = 0
				player_found = 1
			endif
		endif
	endif
