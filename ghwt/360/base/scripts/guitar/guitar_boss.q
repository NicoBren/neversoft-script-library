...
Boss_Ted_Props = {
	...
	character_profile = TedNugent
	character_name = qs("Ted")
	cutoff_beginner = [
		25000
		75000
		200000
...
Boss_Zakk_Props = {
	...
	character_profile = ZakkWylde
	character_name = qs("Zakk")
	cutoff_beginner = [
		50000
		100000
		150000
...
script bossbattle_init 
	if ($game_mode = p2_battle)
		ScriptAssert \{qs("\LCannot choose p2_battle and bossbattle")}
	endif
...
script is_boss_battle_song song = ($current_song)
...
script bossbattle_ready \{battle_gem = 0
	...
	if ($<player_status>.lefthanded_button_ups = 1)
		begin_pos = ((($button_up_models [(<player> -1)]).<color>.left_pos_2d) - (0.0, 90.0))
	else
		begin_pos = ((($button_up_models [(<player> -1)]).<color>.pos_2d) - (0.0, 90.0))
	endif
	if ($<player_status>.player = 1)
...
script bossbattle_trigger_on \{player = 1
		player_text = 'p1'
		player_status = player1_status}
	printf \{qs("\Lbossbattle_trigger_on")}
	if ($<player_status>.current_num_powerups = 0)
		return
	endif
	if ($<player_status>.player = 1)
...
script bossbattle_ai_damage \{player_status = player2_status
	...
	missed_note_percentage = ($current_boss.PowerUpMissedNote.($player1_status.difficulty))
	if ($battlemode_powerups [<select>].name = BrokenString)
		<end_creation_time> = (<start_creation_time> + 60000)
		<missed_note_percentage> = ($current_boss.BrokenStringMissedNote.($player1_status.difficulty))
	endif
	begin
	begin
	GetSongTimeMs
...
script boss_battle_death_icon 
	...
	LegacyDoScreenElementMorph id = <id> pos = <player_pos> scale = 3.1 relative_scale time = 1.0
	Wait \{2.0
		seconds}
	LegacyDoScreenElementMorph id = <id> alpha = 0 time = 2.0
	Wait \{2.0
		seconds}
...
script create_battle_death_meter 
	...
	LegacyDoScreenElementMorph \{id = battle_death_meter
		pos = (648.0, 500.0)
		time = 0.3}
	Wait \{0.3
		seconds}
...
script update_battle_death_meter 
	...
	if ScreenElementExists id = <death_meter_marker>
		if (<color_update> = 0)
			LegacyDoScreenElementMorph id = <death_meter_marker> rgba = [0 255 100 255] time = 1
			<color_update> = 1
		else
			LegacyDoScreenElementMorph id = <death_meter_marker> rgba = [255 255 255 255] time = 1
			<color_update> = 0
		endif
		if (<meterStep> > 0)
			LegacyDoScreenElementMorph id = <death_meter_marker> pos = ((29.0, 200.0) - ((0.0, 1.0) * <pos_update>)) time = <meterStep>
		endif
	else
		break
	endif
...
script update_battle_death_meter_wings 
	...
	if ScreenElementExists id = <wing_R>
		if (<meterTime> > 0)
			LegacyDoScreenElementMorph id = <wing_R> rot_angle = (15 - <rot_update>) time = <meterStep>
		endif
	else
		break
	endif
	if ScreenElementExists id = <wing_L>
		if (<meterTime> > 0)
			LegacyDoScreenElementMorph id = <wing_L> rot_angle = (-15 + <rot_update>) time = <meterStep>
		endif
	else
		break
	endif
