...
script gem_scroller \{player = 1
	...
	elseif (($<player_status>.bot_play = 1) || ($<player_status>.is_local_client = 0))
		<do_bot> = 1
	endif
	if (<do_bot> = 1)
...
script addlagoffsets 
	...
	GetPlatform
	if NOT NotIsMacPort
		if StructureContains Structure = <settings> name = macintosh
			<platform> = macintosh
		endif
	endif
...
script load_perm_ingame_anims 
	mpm_load_pak pak = perm_anims_in_game async = <async>
	mpm_unload_pak pak = perm_anims_in_game async = <async> already_unloaded_is_okay
endscript
