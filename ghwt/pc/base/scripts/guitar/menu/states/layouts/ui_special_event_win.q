...
script create_special_event_win_menu \{quit_segment = 0}
	...
	if (<no_time_left> = 1)
		CreateScreenElement \{type = TextElement
			parent = sewin_container
			font = fontgrid_text_a3
			text = qs("NO TIME LEFT TO COMPLETE SECTIONS!")
			pos = (640.0, 150.0)
			just = [
				center
				center
	...
	if (<show_song_stats> = 1)
		...
		CreateScreenElement {
			...
			parent = sewin_container
			font = fontgrid_text_a3
			text = (qs("Remaining Time: ") + <time_formatted>)
			pos = (640.0, 250.0)
			just = [center center]
			rgba = [200 200 200 255]
		}
		...
		if (<section_index> >= $practice_start_index && (<section_index> + 1) <= $practice_end_index)
			...
			if (<show_pass_or_fail> = 1)
				...
				FormatText TextName = section_and_percentage qs("\L%a - %b") a = <section_name> b = <new_note_percentage>
				<section_and_percentage> = (<section_and_percentage> + qs("\L%"))
				...
				elseif (<new_note_percentage> = 100 && <note_missed> = 1)
					...
					<failed_or_passed_text> = (<failed_or_passed_text> + qs(" - Hit an extra note"))
				endif
