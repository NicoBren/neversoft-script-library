...
current_level = load_z_soundcheck
...
Cheat_BucketHat = -1
BX_Game_Title = 'Smash Hits'
BX_Short_Game_Title = 'SH'
...
script IsGreatestHits 
	if IsPs3
		return \{is_greatest_hits = 0}
	else
		GetTerritory
		if ((<territory> = territory_europe) || (<territory> = territory_undefined))
			return \{is_greatest_hits = 1}
		else
			return \{is_greatest_hits = 0}
		endif
	endif
...
script guitar_startup 
	...
	GetStartTime
	print_loading_time StartTime = <StartTime> text = qs("\Lpak/global_model_tex/global_model_tex.pak")
	PopMemProfile
	PushMemProfile \{'Game Heaps'}
	MemPushContext \{heap_cas}
	GetStartTime
	print_loading_time StartTime = <StartTime> text = qs("\Lpak/cas/cas_deform.pak")
	GetStartTime
	print_loading_time StartTime = <StartTime> text = qs("\Lpak/cas/cas_acc_bones.pak")
	GetStartTime
	CASAddGlobalRawPak \{name = `pak/cas/cas_globalpak.pak`}
	CASAddGlobalRawPak \{name = `pak/cas/cas_globalpak_vram.pak`}
	print_loading_time StartTime = <StartTime> text = qs("\Lpak/cas/cas_globalpak.pak & pak/cas_captextures.pak")
	create_zone_maps
...
script set_bink_heap_state 
	...
	switch ($bink_heap_state)
		case off
		printf \{qs("\Lset_bink_heap_state - doing nothing")}
		case big
		case small
		memdeleteheap \{name = heap_bink}
		default
		ScriptAssert \{qs("\Lset_bink_heap_state - unknown unload state")}
	endswitch
	change bink_heap_state = <state>
	switch (<state>)
		case small
		printf \{qs("\Lset_bink_heap_state - heap is SMALL")}
		MemInitHeap name = 'heap_bink' size = <heap_size_bink_small>
		case big
		printf \{qs("\Lset_bink_heap_state - heap is BIG")}
		MemInitHeap name = 'heap_bink' size = <heap_size_bink>
		case off
		printf \{qs("\Lset_bink_heap_state - heap is OFF")}
		default
		ScriptAssert \{qs("\Lset_bink_heap_state - unknown load state")}
	endswitch
	MemPopContext
endscript
