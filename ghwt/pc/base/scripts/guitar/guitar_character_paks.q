Highway_Pakman_Links = {
	highway_axel = {
		name = 'highway_axel'
	}
	highway_casey = {
		name = 'highway_casey'
	}
	highway_Izzy = {
		name = 'highway_izzy'
	}
	highway_Johnny = {
		name = 'highway_johnny'
	}
	highway_judy = {
		name = 'highway_judy'
	}
	highway_Lars = {
		name = 'highway_lars'
	}
	highway_Midori = {
		name = 'highway_midori'
	}
	highway_classic = {
		name = 'highway_classic'
	}
	highway_barker = {
		name = 'highway_barker'
	}
	highway_corgan = {
		name = 'highway_corgan'
	}
	highway_nugent = {
		name = 'highway_Nugent'
	}
	highway_hendrix = {
		name = 'highway_Hendrix'
	}
	highway_rockbot = {
		name = 'highway_Rockbot'
	}
	highway_skeleton = {
		name = 'highway_Skeleton'
	}
	highway_sting = {
		name = 'highway_Sting'
	}
	highway_Winston = {
		name = 'highway_Winston'
	}
	highway_wylde = {
		name = 'highway_Wylde'
	}
}
frontend_anim_path = 'frontend/'
guit_loop_anim_path = 'ingame/Loops/GUIT/'
sing_loop_anim_path = 'ingame/Loops/SING/'
drum_loop_anim_path = 'ingame/Loops/DRUM/'
intro_anim_path = 'ingame/Intros/'
win_anim_path = 'ingame/Wins/'
lose_anim_path = 'ingame/Lose/'
point_win_anim_path = 'ingame/PointWin/'
point_lose_anim_path = 'ingame/PointLose/'
starpower_anim_path = 'ingame/StarPower/'
Anim_Perm_Pakman_Links = {
	perm_anims_in_game = {
		name = 'perm_anims_in_game'
		path = 'perm_anims/'
	}
	frontend_perm_anims = {
		name = 'frontend_perm_anims'
		path = $frontend_anim_path
	}
}
Anim_Loop_GUIT_SINGER_Pakman_Links = {
	L_GUIT_ChrisVance_Bulls_anims = {
		name = 'L_GUIT_ChrisVance_Bulls_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_ChrisVance_Damnit_anims = {
		name = 'L_GUIT_ChrisVance_Damnit_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_ChrisV_Damnit2_anims = {
		name = 'L_GUIT_ChrisV_Damnit2_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_ChrisVance_Joker_anims = {
		name = 'L_GUIT_ChrisVance_Joker_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Davidicus_Damnit_anims = {
		name = 'L_GUIT_Davidicus_Damnit_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Davidicus_Damnit2_anims = {
		name = 'L_GUIT_Davidicus_Damnit2_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Davidicus_Joker_anims = {
		name = 'L_GUIT_Davidicus_Joker_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Davidicus_Bulls_anims = {
		name = 'L_GUIT_Davidicus_Bulls_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Dvdicus_Bulls_150_anims = {
		name = 'L_GUIT_Dvdicus_Bulls_150_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Dan_Damnit_anims = {
		name = 'L_GUIT_Dan_Damnit_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Dan_Joker_anims = {
		name = 'L_GUIT_Dan_Joker_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_JoeR_AreYou_anims = {
		name = 'L_GUIT_JoeR_AreYou_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_JoeR_Hot_anims = {
		name = 'L_GUIT_JoeR_Hot_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_JeffS_LowKey_anims = {
		name = 'L_GUIT_JeffS_LowKey_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_JeffS_LowKey2_anims = {
		name = 'L_GUIT_JeffS_LowKey2_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Jimi_Joker_anims = {
		name = 'L_GUIT_Jimi_Joker_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_JoeS_AreYou_anims = {
		name = 'L_GUIT_JoeS_AreYou_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_JoeS_Hot_anims = {
		name = 'L_GUIT_JoeS_Hot_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_JoeS_Hot_150_anims = {
		name = 'L_GUIT_JoeS_Hot_150_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Matt_Stillborn_anims = {
		name = 'L_GUIT_Matt_Stillborn_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Matt_Bulls_anims = {
		name = 'L_GUIT_Matt_Bulls_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Sam_Joker_anims = {
		name = 'L_GUIT_Sam_Joker_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Sam_Dammit_anims = {
		name = 'L_GUIT_Sam_Dammit_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Sonny_Bulls_anims = {
		name = 'L_GUIT_Sonny_Bulls_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Sonny_Stillborn_anims = {
		name = 'L_GUIT_Sonny_Stillborn_anims'
		path = $guit_loop_anim_path
	}
	l_guit_sting_demolition_anims = {
		name = 'L_GUIT_Sting_Demolition_anims'
		path = $guit_loop_anim_path
	}
	l_guit_zakk_stillborn_anims = {
		name = 'L_GUIT_Zakk_Stillborn_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Ginger_AreYou_anims = {
		name = 'L_GUIT_Ginger_AreYou_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Ginger_Bulls_anims = {
		name = 'L_GUIT_Ginger_Bulls_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Ginger_Joker_anims = {
		name = 'L_GUIT_Ginger_Joker_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Judita_AreYou_anims = {
		name = 'L_GUIT_Judita_AreYou_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Judita_Bulls_anims = {
		name = 'L_GUIT_Judita_Bulls_anims'
		path = $guit_loop_anim_path
	}
	L_GUIT_Judita_Joker_anims = {
		name = 'L_GUIT_Judita_Joker_anims'
		path = $guit_loop_anim_path
	}
	l_guit_gsb_b_floaton = {
		name = 'L_GUIT_GSB_B_FloatOn'
		path = $guit_loop_anim_path
	}
	l_guit_gsb_g_floaton = {
		name = 'L_GUIT_GSB_G_FloatOn'
		path = $guit_loop_anim_path
	}
	frontend_anims_guit1 = {
		name = 'frontend_anims_guit1'
		path = $frontend_anim_path
	}
	frontend_anims_guit2 = {
		name = 'frontend_anims_guit2'
		path = $frontend_anim_path
	}
	L_SING_JeffS_LowKey_anims = {
		name = 'L_SING_JeffS_LowKey_anims'
		path = $sing_loop_anim_path
	}
	L_SING_JeffS_LowKey2_anims = {
		name = 'L_SING_JeffS_LowKey2_anims'
		path = $sing_loop_anim_path
	}
	L_SING_JoeH_Hot_anims = {
		name = 'L_SING_JoeH_Hot_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Josh_Dammit_anims = {
		name = 'L_SING_Josh_Dammit_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Josh_Joker_anims = {
		name = 'L_SING_Josh_Joker_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Rob_Bulls_anims = {
		name = 'L_SING_Rob_Bulls_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Rob_Stillborn_anims = {
		name = 'L_SING_Rob_Stillborn_anims'
		path = $sing_loop_anim_path
	}
	L_SING_RockG_Damnit_anims = {
		name = 'L_SING_RockG_Damnit_anims'
		path = $sing_loop_anim_path
	}
	L_SING_RockG_Damnit2_anims = {
		name = 'L_SING_RockG_Damnit2_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Rockg_Joker_anims = {
		name = 'L_SING_RockG_Joker_anims'
		path = $sing_loop_anim_path
	}
	L_SING_RockG_Bulls_anims = {
		name = 'L_SING_RockG_Bulls_anims'
		path = $sing_loop_anim_path
	}
	L_SING_JoeH_AreYou_anims = {
		name = 'L_SING_JoeH_AreYou_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Natalie_AreYou_anims = {
		name = 'L_SING_Natalie_AreYou_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Haley_Bulls_anims = {
		name = 'L_SING_Haley_Bulls_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Haley_Dammit_anims = {
		name = 'L_SING_Haley_Dammit_anims'
		path = $sing_loop_anim_path
	}
	L_SING_Haley_Joker_anims = {
		name = 'L_SING_Haley_Joker_anims'
		path = $sing_loop_anim_path
	}
	l_sing_gsb_s_floaton = {
		name = 'L_SING_GSB_S_FloatOn'
		path = $sing_loop_anim_path
	}
	frontend_anims_sing = {
		name = 'frontend_anims_sing'
		path = $frontend_anim_path
	}
}
Anim_Loop_DRUM_Pakman_Links = {
	L_DRUM_Loops_Standard_anims = {
		name = 'L_DRUM_Loops_Standard_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_Misery_anims = {
		name = 'L_DRUM_Loops_Misery_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_BYOB_anims = {
		name = 'L_DRUM_Loops_BYOB_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_Soul_Doubt_anims = {
		name = 'L_DRUM_Loops_Soul_Doubt_anims'
		path = $drum_loop_anim_path
	}
	l_drum_loops_1armdscior_anims = {
		name = 'L_DRUM_Loops_1ArmdScior_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_Overkill_anims = {
		name = 'L_DRUM_Loops_Overkill_anims'
		path = $drum_loop_anim_path
	}
	l_drum_loops_pullmeunder_anims = {
		name = 'L_DRUM_Loops_PullMeUnder_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_FrkOnLsh_anims = {
		name = 'L_DRUM_Loops_FrkOnLsh_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_JessiesGrl_anims = {
		name = 'L_DRUM_Loops_JessiesGrl_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_Satchboogie_anims = {
		name = 'L_DRUM_Loops_Satchboogie_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_scream_anims = {
		name = 'L_DRUM_Loops_scream_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_kill_anims = {
		name = 'L_DRUM_Loops_kill_anims'
		path = $drum_loop_anim_path
	}
	L_DRUM_Loops_FloatOn_anims = {
		name = 'L_DRUM_Loops_FloatOn_anims'
		path = $drum_loop_anim_path
	}
	frontend_anims_drum = {
		name = 'frontend_anims_drum'
		path = $frontend_anim_path
	}
}
Anim_Intro_Win_Pakman_Links = {
	d_travis_win = {
		name = 'd_travis_win'
		path = $win_anim_path
	}
	g_jimi_win_01 = {
		name = 'g_jimi_win_01'
		path = $win_anim_path
	}
	g_ted_win = {
		name = 'g_ted_win'
		path = $win_anim_path
	}
	g_ted_win_01 = {
		name = 'g_ted_win_01'
		path = $win_anim_path
	}
	guit_zakk_winl_02 = {
		name = 'Guit_Zakk_WinL_02'
		path = $win_anim_path
	}
	guit_zakk_winr = {
		name = 'Guit_Zakk_WinR'
		path = $win_anim_path
	}
	s_billy_intro_boss = {
		name = 's_billy_intro_boss'
		path = $intro_anim_path
	}
	S_Billy_Win_Boss = {
		name = 'S_Billy_Win_Boss'
		path = $win_anim_path
	}
	s_sting_win_01 = {
		name = 's_sting_win_01'
		path = $win_anim_path
	}
	s_ozzy_win = {
		name = 's_ozzy_win'
		path = $win_anim_path
	}
	s_ozzy_intro_01 = {
		name = 's_ozzy_intro_01'
		path = $intro_anim_path
	}
	s_ozzy_intro_02 = {
		name = 's_ozzy_intro_02'
		path = $intro_anim_path
	}
	s_haley_win = {
		name = 's_haley_win'
		path = $win_anim_path
	}
	d_travis_win = {
		name = 'd_travis_win'
		path = $win_anim_path
	}
	d_travis_win_pointbass = {
		name = 'd_travis_win_pointbass'
		path = $win_anim_path
	}
	d_travis_win_pointguitar = {
		name = 'd_travis_win_pointguitar'
		path = $win_anim_path
	}
	d_travis_win_pointsinger = {
		name = 'd_travis_win_pointsinger'
		path = $win_anim_path
	}
	g_jimi_win_01 = {
		name = 'g_jimi_win_01'
		path = $win_anim_path
	}
	g_jimi_win_pointdrummer = {
		name = 'g_jimi_win_pointdrummer'
		path = $win_anim_path
	}
	g_jimi_win_pointl = {
		name = 'g_jimi_win_pointl'
		path = $win_anim_path
	}
	g_ted_win = {
		name = 'g_ted_win'
		path = $win_anim_path
	}
	g_ted_win_01 = {
		name = 'g_ted_win_01'
		path = $win_anim_path
	}
	g_ted_win_pointbass = {
		name = 'g_ted_win_pointbass'
		path = $win_anim_path
	}
	g_ted_win_pointdrum = {
		name = 'g_ted_win_pointdrum'
		path = $win_anim_path
	}
	g_ted_win_pointsinger = {
		name = 'g_ted_win_pointsinger'
		path = $win_anim_path
	}
	guit_zakk_winl_02 = {
		name = 'Guit_Zakk_WinL_02'
		path = $win_anim_path
	}
	guit_zakk_winr = {
		name = 'Guit_Zakk_WinR'
		path = $win_anim_path
	}
	S_Billy_Win_Boss = {
		name = 'S_Billy_Win_Boss'
		path = $win_anim_path
	}
	s_haley_win = {
		name = 's_haley_win'
		path = $win_anim_path
	}
	s_haley_win_pointbass = {
		name = 's_haley_win_pointbass'
		path = $win_anim_path
	}
	s_haley_win_pointdrum = {
		name = 's_haley_win_pointdrum'
		path = $win_anim_path
	}
	s_haley_win_pointguitar = {
		name = 's_haley_win_pointguitar'
		path = $win_anim_path
	}
	s_ozzy_win = {
		name = 's_ozzy_win'
		path = $win_anim_path
	}
	s_ozzy_win_pointguitar = {
		name = 's_ozzy_win_pointguitar'
		path = $point_win_anim_path
	}
	s_ozzy_win_pointbass = {
		name = 's_ozzy_win_pointbass'
		path = $point_win_anim_path
	}
	s_ozzy_win_pointdrum = {
		name = 's_ozzy_win_pointdrum'
		path = $point_win_anim_path
	}
	s_sting_win_01 = {
		name = 's_sting_win_01'
		path = $win_anim_path
	}
	s_sting_win_pointbass = {
		name = 's_sting_win_pointbass'
		path = $win_anim_path
	}
	s_sting_win_pointdrum = {
		name = 's_sting_win_pointdrum'
		path = $win_anim_path
	}
	s_sting_win_pointguitar = {
		name = 's_sting_win_pointguitar'
		path = $win_anim_path
	}
	b_june_win_pointbass = {
		name = 'b_june_win_pointbass'
		path = $win_anim_path
	}
	b_june_win_pointdrum = {
		name = 'b_june_win_pointdrum'
		path = $win_anim_path
	}
	b_june_win_pointguit = {
		name = 'b_june_win_pointguit'
		path = $win_anim_path
	}
	b_june_win_pointsing = {
		name = 'b_june_win_pointsing'
		path = $win_anim_path
	}
	d_june_win_pointbass = {
		name = 'd_june_win_pointbass'
		path = $win_anim_path
	}
	d_june_win_pointdrum = {
		name = 'd_june_win_pointdrum'
		path = $win_anim_path
	}
	d_june_win_pointguit = {
		name = 'd_june_win_pointguit'
		path = $win_anim_path
	}
	d_june_win_pointsing = {
		name = 'd_june_win_pointsing'
		path = $win_anim_path
	}
	g_june_win_pointbass = {
		name = 'g_june_win_pointbass'
		path = $win_anim_path
	}
	g_june_win_pointdrum = {
		name = 'g_june_win_pointdrum'
		path = $win_anim_path
	}
	g_june_win_pointguit = {
		name = 'g_june_win_pointguit'
		path = $win_anim_path
	}
	g_june_win_pointsing = {
		name = 'g_june_win_pointsing'
		path = $win_anim_path
	}
	s_june_win_pointbass = {
		name = 's_june_win_pointbass'
		path = $win_anim_path
	}
	s_june_win_pointdrum = {
		name = 's_june_win_pointdrum'
		path = $win_anim_path
	}
	s_june_win_pointguit = {
		name = 's_june_win_pointguit'
		path = $win_anim_path
	}
	s_june_win_pointsing = {
		name = 's_june_win_pointsing'
		path = $win_anim_path
	}
	b_alla_win_generic = {
		name = 'b_alla_win_generic'
		path = $win_anim_path
	}
	b_alla_win_jump = {
		name = 'b_alla_win_jump'
		path = $win_anim_path
	}
	b_jeff_win_adjust = {
		name = 'b_jeff_win_adjust'
		path = $win_anim_path
	}
	b_jeff_win_kick = {
		name = 'b_jeff_win_kick'
		path = $win_anim_path
	}
	b_jeff_win_KungFu = {
		name = 'b_jeff_win_kungfu'
		path = $win_anim_path
	}
	b_jeff_win_pretentious = {
		name = 'b_jeff_win_pretentious'
		path = $win_anim_path
	}
	b_jeff_win_robot = {
		name = 'b_jeff_win_robot'
		path = $win_anim_path
	}
	b_jeff_win_Scary = {
		name = 'b_jeff_win_scary'
		path = $win_anim_path
	}
	b_jeff_win_stagedive = {
		name = 'b_jeff_win_stagedive'
		path = $win_anim_path
	}
	b_jeff_win_waving = {
		name = 'b_jeff_win_waving'
		path = $win_anim_path
	}
	b_kris_win_crowdhype01 = {
		name = 'b_kris_win_crowdhype01'
		path = $win_anim_path
	}
	b_kris_win_elvis = {
		name = 'b_kris_win_elvis'
		path = $win_anim_path
	}
	b_kris_win_kick = {
		name = 'b_kris_win_kick'
		path = $win_anim_path
	}
	b_kris_win_kungfu = {
		name = 'b_kris_win_kungfu'
		path = $win_anim_path
	}
	b_kris_win_pointbass = {
		name = 'b_kris_win_pointbass'
		path = $win_anim_path
	}
	b_kris_win_pointdrum = {
		name = 'b_kris_win_pointdrum'
		path = $win_anim_path
	}
	b_kris_win_pointguit = {
		name = 'b_kris_win_pointguit'
		path = $win_anim_path
	}
	b_kris_win_pointsing = {
		name = 'b_kris_win_pointsing'
		path = $win_anim_path
	}
	b_kris_win_pretentious = {
		name = 'b_kris_win_pretentious'
		path = $win_anim_path
	}
	b_kris_win_robot = {
		name = 'b_kris_win_robot'
		path = $win_anim_path
	}
	b_kris_win_scary = {
		name = 'b_kris_win_scary'
		path = $win_anim_path
	}
	b_kris_win_stagedive = {
		name = 'b_kris_win_stagedive'
		path = $win_anim_path
	}
	b_kris_win_waving = {
		name = 'b_kris_win_waving'
		path = $win_anim_path
	}
	b_ringers_take03_wins01 = {
		name = 'b_ringers_take03_wins01'
		path = $win_anim_path
	}
	car_female_wins_crowdhype = {
		name = 'car_female_wins_crowdhype'
		path = $win_anim_path
	}
	car_female_wins_elvis = {
		name = 'car_female_wins_elvis'
		path = $win_anim_path
	}
	car_female_wins_generic = {
		name = 'car_female_wins_generic'
		path = $win_anim_path
	}
	CAR_Female_Wins_Jump = {
		name = 'car_female_wins_jump'
		path = $win_anim_path
	}
	CAR_Female_Wins_KungFu = {
		name = 'car_female_wins_kungfu'
		path = $win_anim_path
	}
	car_female_wins_robot = {
		name = 'car_female_wins_robot'
		path = $win_anim_path
	}
	car_female_wins_stagedive = {
		name = 'car_female_wins_stagedive'
		path = $win_anim_path
	}
	car_wins_crowdhype = {
		name = 'car_wins_crowdhype'
		path = $win_anim_path
	}
	car_wins_elvis = {
		name = 'car_wins_elvis'
		path = $win_anim_path
	}
	car_wins_generic = {
		name = 'car_wins_generic'
		path = $win_anim_path
	}
	car_wins_generic_02 = {
		name = 'car_wins_generic_02'
		path = $win_anim_path
	}
	car_wins_generic_03 = {
		name = 'car_wins_generic_03'
		path = $win_anim_path
	}
	car_wins_kicks = {
		name = 'car_wins_kicks'
		path = $win_anim_path
	}
	car_wins_KungFu = {
		name = 'car_wins_kungfu'
		path = $win_anim_path
	}
	CAR_wins_noinstrument = {
		name = 'car_wins_noinstrument'
		path = $win_anim_path
	}
	car_wins_pointing = {
		name = 'car_wins_pointing'
		path = $win_anim_path
	}
	car_wins_pretentious = {
		name = 'car_wins_pretentious'
		path = $win_anim_path
	}
	car_wins_robot = {
		name = 'car_wins_robot'
		path = $win_anim_path
	}
	car_wins_Scary = {
		name = 'car_wins_scary'
		path = $win_anim_path
	}
	car_wins_waving = {
		name = 'car_wins_waving'
		path = $win_anim_path
	}
	d_alla_win_generic = {
		name = 'd_alla_win_generic'
		path = $win_anim_path
	}
	d_alla_win_jump = {
		name = 'd_alla_win_jump'
		path = $win_anim_path
	}
	d_jeff_win_adjust = {
		name = 'd_jeff_win_adjust'
		path = $win_anim_path
	}
	d_jeff_win_kick = {
		name = 'd_jeff_win_kick'
		path = $win_anim_path
	}
	d_jeff_win_KungFu = {
		name = 'd_jeff_win_kungfu'
		path = $win_anim_path
	}
	d_jeff_win_pretentious = {
		name = 'd_jeff_win_pretentious'
		path = $win_anim_path
	}
	d_jeff_win_robot = {
		name = 'd_jeff_win_robot'
		path = $win_anim_path
	}
	d_jeff_win_Scary = {
		name = 'd_jeff_win_scary'
		path = $win_anim_path
	}
	d_jeff_win_stagedive = {
		name = 'd_jeff_win_stagedive'
		path = $win_anim_path
	}
	d_jeff_win_waving = {
		name = 'd_jeff_win_waving'
		path = $win_anim_path
	}
	d_kris_win_crowdhype01 = {
		name = 'd_kris_win_crowdhype01'
		path = $win_anim_path
	}
	d_kris_win_elvis = {
		name = 'd_kris_win_elvis'
		path = $win_anim_path
	}
	d_kris_win_kick = {
		name = 'd_kris_win_kick'
		path = $win_anim_path
	}
	d_kris_win_kungfu = {
		name = 'd_kris_win_kungfu'
		path = $win_anim_path
	}
	d_kris_win_pointdrum = {
		name = 'd_kris_win_pointdrum'
		path = $win_anim_path
	}
	d_kris_win_pointguit = {
		name = 'd_kris_win_pointguit'
		path = $win_anim_path
	}
	d_kris_win_pretentious = {
		name = 'd_kris_win_pretentious'
		path = $win_anim_path
	}
	d_kris_win_robot = {
		name = 'd_kris_win_robot'
		path = $win_anim_path
	}
	d_kris_win_scary = {
		name = 'd_kris_win_scary'
		path = $win_anim_path
	}
	d_kris_win_stagedive = {
		name = 'd_kris_win_stagedive'
		path = $win_anim_path
	}
	d_kris_win_waving = {
		name = 'd_kris_win_waving'
		path = $win_anim_path
	}
	d_ringers_take02_wins01 = {
		name = 'd_ringers_take02_wins01'
		path = $win_anim_path
	}
	d_ringers_take03_wins02 = {
		name = 'd_ringers_take03_wins02'
		path = $win_anim_path
	}
	gsbd_b_win_crowdhype01 = {
		name = 'gsbd_b_win_crowdhype01'
		path = $win_anim_path
	}
	gsbd_b_win_elvis01 = {
		name = 'gsbd_b_win_elvis01'
		path = $win_anim_path
	}
	gsbd_d_win_crowdhype01 = {
		name = 'gsbd_d_win_crowdhype01'
		path = $win_anim_path
	}
	gsbd_d_win_elvis01 = {
		name = 'gsbd_d_win_elvis01'
		path = $win_anim_path
	}
	gsbd_d_win_pointing01 = {
		name = 'gsbd_d_win_pointing01'
		path = $win_anim_path
	}
	gsbd_g_win_crowdhype01 = {
		name = 'gsbd_g_win_crowdhype01'
		path = $win_anim_path
	}
	gsbd_g_win_elvis01 = {
		name = 'gsbd_g_win_elvis01'
		path = $win_anim_path
	}
	gsbd_s_win_crowdhype01 = {
		name = 'gsbd_s_win_crowdhype01'
		path = $win_anim_path
	}
	gsbd_s_win_elvis01 = {
		name = 'gsbd_s_win_elvis01'
		path = $win_anim_path
	}
	g_alla_win_generic = {
		name = 'g_alla_win_generic'
		path = $win_anim_path
	}
	g_alla_win_jump = {
		name = 'g_alla_win_jump'
		path = $win_anim_path
	}
	g_jeff_win_adjust = {
		name = 'g_jeff_win_adjust'
		path = $win_anim_path
	}
	g_jeff_win_kick = {
		name = 'g_jeff_win_kick'
		path = $win_anim_path
	}
	g_jeff_win_KungFu = {
		name = 'g_jeff_win_kungfu'
		path = $win_anim_path
	}
	g_jeff_win_pretentious = {
		name = 'g_jeff_win_pretentious'
		path = $win_anim_path
	}
	g_jeff_win_robot = {
		name = 'g_jeff_win_robot'
		path = $win_anim_path
	}
	g_jeff_win_Scary = {
		name = 'g_jeff_win_scary'
		path = $win_anim_path
	}
	g_jeff_win_stagedive = {
		name = 'g_jeff_win_stagedive'
		path = $win_anim_path
	}
	g_jeff_win_waving = {
		name = 'g_jeff_win_waving'
		path = $win_anim_path
	}
	g_kris_win_crowdhype01 = {
		name = 'g_kris_win_crowdhype01'
		path = $win_anim_path
	}
	g_kris_win_elvis = {
		name = 'g_kris_win_elvis'
		path = $win_anim_path
	}
	g_kris_win_kick = {
		name = 'g_kris_win_kick'
		path = $win_anim_path
	}
	g_kris_win_kungfu = {
		name = 'g_kris_win_kungfu'
		path = $win_anim_path
	}
	g_kris_win_pointbass = {
		name = 'g_kris_win_pointbass'
		path = $win_anim_path
	}
	g_kris_win_pointdrum = {
		name = 'g_kris_win_pointdrum'
		path = $win_anim_path
	}
	g_kris_win_pointguit = {
		name = 'g_kris_win_pointguit'
		path = $win_anim_path
	}
	g_kris_win_pointsing = {
		name = 'g_kris_win_pointsing'
		path = $win_anim_path
	}
	g_kris_win_pretentious = {
		name = 'g_kris_win_pretentious'
		path = $win_anim_path
	}
	g_kris_win_robot = {
		name = 'g_kris_win_robot'
		path = $win_anim_path
	}
	g_kris_win_scary = {
		name = 'g_kris_win_scary'
		path = $win_anim_path
	}
	g_kris_win_stagedive = {
		name = 'g_kris_win_stagedive'
		path = $win_anim_path
	}
	g_kris_win_waving = {
		name = 'g_kris_win_waving'
		path = $win_anim_path
	}
	g_ringers_take02_wins01 = {
		name = 'g_ringers_take02_wins01'
		path = $win_anim_path
	}
	g_ringers_take03_wins02 = {
		name = 'g_ringers_take03_wins02'
		path = $win_anim_path
	}
	s_alla_win_generic = {
		name = 's_alla_win_generic'
		path = $win_anim_path
	}
	s_alla_win_jump = {
		name = 's_alla_win_jump'
		path = $win_anim_path
	}
	s_jeff_win_adjust = {
		name = 's_jeff_win_adjust'
		path = $win_anim_path
	}
	s_jeff_win_kick = {
		name = 's_jeff_win_kick'
		path = $win_anim_path
	}
	s_jeff_win_KungFu = {
		name = 's_jeff_win_kungfu'
		path = $win_anim_path
	}
	s_jeff_win_pretentious = {
		name = 's_jeff_win_pretentious'
		path = $win_anim_path
	}
	s_jeff_win_robot = {
		name = 's_jeff_win_robot'
		path = $win_anim_path
	}
	s_jeff_win_Scary = {
		name = 's_jeff_win_scary'
		path = $win_anim_path
	}
	s_jeff_win_stagedive = {
		name = 's_jeff_win_stagedive'
		path = $win_anim_path
	}
	s_jeff_win_waving = {
		name = 's_jeff_win_waving'
		path = $win_anim_path
	}
	s_kris_win_crowdhype01 = {
		name = 's_kris_win_crowdhype01'
		path = $win_anim_path
	}
	s_kris_win_elvis = {
		name = 's_kris_win_elvis'
		path = $win_anim_path
	}
	s_kris_win_kick = {
		name = 's_kris_win_kick'
		path = $win_anim_path
	}
	s_kris_win_kungfu = {
		name = 's_kris_win_kungfu'
		path = $win_anim_path
	}
	s_kris_win_pointbass = {
		name = 's_kris_win_pointbass'
		path = $win_anim_path
	}
	s_kris_win_pointdrum = {
		name = 's_kris_win_pointdrum'
		path = $win_anim_path
	}
	s_kris_win_pointguit = {
		name = 's_kris_win_pointguit'
		path = $win_anim_path
	}
	s_kris_win_pointsing = {
		name = 's_kris_win_pointsing'
		path = $win_anim_path
	}
	s_kris_win_pretentious = {
		name = 's_kris_win_pretentious'
		path = $win_anim_path
	}
	s_kris_win_robot = {
		name = 's_kris_win_robot'
		path = $win_anim_path
	}
	s_kris_win_scary = {
		name = 's_kris_win_scary'
		path = $win_anim_path
	}
	s_kris_win_stagedive = {
		name = 's_kris_win_stagedive'
		path = $win_anim_path
	}
	s_kris_win_waving = {
		name = 's_kris_win_waving'
		path = $win_anim_path
	}
	s_ringers_take02_wins01 = {
		name = 's_ringers_take02_wins01'
		path = $win_anim_path
	}
	s_ringers_take03_wins02 = {
		name = 's_ringers_take03_wins02'
		path = $win_anim_path
	}
	b_june_win_pointbass = {
		name = 'b_june_win_pointbass'
		path = $win_anim_path
	}
	d_june_win_pointdrum = {
		name = 'd_june_win_pointdrum'
		path = $win_anim_path
	}
	g_june_win_pointguit = {
		name = 'g_june_win_pointguit'
		path = $win_anim_path
	}
	s_june_win_pointsing = {
		name = 's_june_win_pointsing'
		path = $win_anim_path
	}
	gsbd_g_win_pointing01 = {
		name = 'gsbd_g_win_pointing01'
		path = $win_anim_path
	}
	gsbd_b_win_pointing01 = {
		name = 'gsbd_b_win_pointing01'
		path = $win_anim_path
	}
	gsbd_d_win_pointing01 = {
		name = 'gsbd_d_win_pointing01'
		path = $win_anim_path
	}
	gsbd_s_win_pointing01 = {
		name = 'gsbd_s_win_pointing01'
		path = $win_anim_path
	}
	gsbd_uj_intro01_b = {
		name = 'gsbd_uj_intro01_b'
		path = $win_anim_path
	}
	gsbd_uj_intro01_d = {
		name = 'gsbd_uj_intro01_d'
		path = $win_anim_path
	}
	gsbd_uj_intro01_g = {
		name = 'gsbd_uj_intro01_g'
		path = $win_anim_path
	}
	gsbd_uj_intro01_s = {
		name = 'gsbd_uj_intro01_s'
		path = $win_anim_path
	}
	g_kris_win_pointsing = {
		name = 'g_kris_win_pointsing'
		path = $win_anim_path
	}
	b_kris_win_pointsing = {
		name = 'b_kris_win_pointsing'
		path = $win_anim_path
	}
	s_kris_win_pointsing = {
		name = 's_kris_win_pointsing'
		path = $win_anim_path
	}
	car_female_wins_pretentious = {
		name = 'CAR_Female_Wins_pretentious'
		path = $win_anim_path
	}
	car_wins_crowdhype = {
		name = 'car_wins_crowdhype'
		path = $win_anim_path
	}
	car_wins_pointing = {
		name = 'car_wins_pointing'
		path = $win_anim_path
	}
	car_wins_pretentious = {
		name = 'car_wins_pretentious'
		path = $win_anim_path
	}
}
Anim_Lose_Pakman_Links = {
	bdgs_b_bassist_lose_01 = {
		name = 'bdgs_b_bassist_lose_01'
		path = $lose_anim_path
	}
	BDGS_B_Drummer_Lose_01 = {
		name = 'bdgs_b_drummer_lose_01'
		path = $lose_anim_path
	}
	BDGS_B_Guitarist_Lose_01 = {
		name = 'bdgs_b_guitarist_lose_01'
		path = $lose_anim_path
	}
	BDGS_B_Singer_Lose_01 = {
		name = 'bdgs_b_singer_lose_01'
		path = $lose_anim_path
	}
	BDGS_D_Bassist_Lose_01 = {
		name = 'bdgs_d_bassist_lose_01'
		path = $lose_anim_path
	}
	bdgs_d_drummer_lose_01 = {
		name = 'bdgs_d_drummer_lose_01'
		path = $lose_anim_path
	}
	BDGS_D_Guitarist_Lose_01 = {
		name = 'bdgs_d_guitarist_lose_01'
		path = $lose_anim_path
	}
	BDGS_D_Singer_Lose_01 = {
		name = 'bdgs_d_singer_lose_01'
		path = $lose_anim_path
	}
	BDGS_G_Bassist_Lose_01 = {
		name = 'bdgs_g_bassist_lose_01'
		path = $lose_anim_path
	}
	BDGS_G_Drummer_Lose_01 = {
		name = 'bdgs_g_drummer_lose_01'
		path = $lose_anim_path
	}
	bdgs_g_guitarist_lose_01 = {
		name = 'bdgs_g_guitarist_lose_01'
		path = $lose_anim_path
	}
	BDGS_G_Singer_Lose_01 = {
		name = 'bdgs_g_singer_lose_01'
		path = $lose_anim_path
	}
	BDGS_S_Bassist_Lose_01 = {
		name = 'bdgs_s_bassist_lose_01'
		path = $lose_anim_path
	}
	BDGS_S_Drummer_Lose_01 = {
		name = 'bdgs_s_drummer_lose_01'
		path = $lose_anim_path
	}
	BDGS_S_Guitarist_Lose_01 = {
		name = 'bdgs_s_guitarist_lose_01'
		path = $lose_anim_path
	}
	bdgs_s_singer_lose_01 = {
		name = 'bdgs_s_singer_lose_01'
		path = $lose_anim_path
	}
	b_jeff_lose_depressed = {
		name = 'b_jeff_lose_depressed'
		path = $lose_anim_path
	}
	b_jeff_lose_fearful = {
		name = 'b_jeff_lose_fearful'
		path = $lose_anim_path
	}
	b_jeff_lose_kickinstrument = {
		name = 'b_jeff_lose_kickinstrument'
		path = $lose_anim_path
	}
	b_jeff_lose_pretentious = {
		name = 'b_jeff_lose_pretentious'
		path = $lose_anim_path
	}
	b_jeff_lose_walkoff = {
		name = 'b_jeff_lose_walkoff'
		path = $lose_anim_path
	}
	B_Kris_Lose_AngryAtCrowd = {
		name = 'B_Kris_Lose_AngryAtCrowd'
		path = $lose_anim_path
	}
	B_Kris_Lose_Depressed = {
		name = 'B_Kris_Lose_Depressed'
		path = $lose_anim_path
	}
	B_Kris_Lose_PointBass = {
		name = 'B_Kris_Lose_PointBass'
		path = $lose_anim_path
	}
	B_Kris_Lose_PointDrum = {
		name = 'B_Kris_Lose_PointDrum'
		path = $lose_anim_path
	}
	B_Kris_Lose_PointGuit = {
		name = 'B_Kris_Lose_PointGuit'
		path = $lose_anim_path
	}
	B_Kris_Lose_PointSing = {
		name = 'B_Kris_Lose_PointSing'
		path = $lose_anim_path
	}
	b_kris_lose_pretentious = {
		name = 'b_kris_lose_pretentious'
		path = $lose_anim_path
	}
	b_kris_lose_tantrum = {
		name = 'b_kris_lose_tantrum'
		path = $lose_anim_path
	}
	CAR_Female_Lose_AngryAtCrowd = {
		name = 'CAR_Female_Lose_AngryAtCrowd'
		path = $lose_anim_path
	}
	CAR_Female_Lose_Depressed = {
		name = 'CAR_Female_Lose_Depressed'
		path = $lose_anim_path
	}
	car_lose_angryatcrowd = {
		name = 'car_lose_angryatcrowd'
		path = $lose_anim_path
	}
	car_lose_depressed = {
		name = 'CAR_lose_depressed'
		path = $lose_anim_path
	}
	car_lose_generic = {
		name = 'car_lose_generic'
		path = $lose_anim_path
	}
	car_lose_pretentious = {
		name = 'car_lose_pretentious'
		path = $lose_anim_path
	}
	car_lose_smash = {
		name = 'car_lose_smash'
		path = $lose_anim_path
	}
	car_lose_tantrum = {
		name = 'car_lose_tantrum'
		path = $lose_anim_path
	}
	d_jeff_lose_depressed = {
		name = 'd_jeff_lose_depressed'
		path = $lose_anim_path
	}
	d_jeff_lose_fearful = {
		name = 'd_jeff_lose_fearful'
		path = $lose_anim_path
	}
	d_jeff_lose_kickinstrument = {
		name = 'd_jeff_lose_kickinstrument'
		path = $lose_anim_path
	}
	d_jeff_lose_pretentious = {
		name = 'd_jeff_lose_pretentious'
		path = $lose_anim_path
	}
	d_jeff_lose_walkoff = {
		name = 'd_jeff_lose_walkoff'
		path = $lose_anim_path
	}
	D_Kris_Lose_AngryAtCrowd = {
		name = 'D_Kris_Lose_AngryAtCrowd'
		path = $lose_anim_path
	}
	D_Kris_Lose_Depressed = {
		name = 'D_Kris_Lose_Depressed'
		path = $lose_anim_path
	}
	D_Kris_Lose_PointBass = {
		name = 'D_Kris_Lose_PointBass'
		path = $lose_anim_path
	}
	D_Kris_Lose_PointDrum = {
		name = 'D_Kris_Lose_PointDrum'
		path = $lose_anim_path
	}
	D_Kris_Lose_PointGuit = {
		name = 'D_Kris_Lose_PointGuit'
		path = $lose_anim_path
	}
	D_Kris_Lose_PointSing = {
		name = 'D_Kris_Lose_PointSing'
		path = $lose_anim_path
	}
	d_kris_lose_pretentious = {
		name = 'd_kris_lose_pretentious'
		path = $lose_anim_path
	}
	d_kris_lose_tantrum = {
		name = 'd_kris_lose_tantrum'
		path = $lose_anim_path
	}
	d_travis_lose_01 = {
		name = 'd_travis_lose_01'
		path = $lose_anim_path
	}
	d_travis_lose_02 = {
		name = 'd_travis_lose_02'
		path = $lose_anim_path
	}
	d_travis_lose_pointbass = {
		name = 'd_travis_lose_pointbass'
		path = $lose_anim_path
	}
	d_travis_lose_pointguitarist = {
		name = 'd_travis_lose_pointguitarist'
		path = $lose_anim_path
	}
	d_travis_lose_pointsinger = {
		name = 'd_travis_lose_pointsinger'
		path = $lose_anim_path
	}
	gsbd_b_lose_angryatcrowd01 = {
		name = 'gsbd_b_lose_angryatcrowd01'
		path = $lose_anim_path
	}
	gsbd_b_lose_tantrum01 = {
		name = 'gsbd_b_lose_tantrum01'
		path = $lose_anim_path
	}
	gsbd_d_lose_angryatcrowd01 = {
		name = 'gsbd_d_lose_angryatcrowd01'
		path = $lose_anim_path
	}
	gsbd_d_lose_tantrum01 = {
		name = 'gsbd_d_lose_tantrum01'
		path = $lose_anim_path
	}
	gsbd_g_lose_angryatcrowd01 = {
		name = 'gsbd_g_lose_angryatcrowd01'
		path = $lose_anim_path
	}
	gsbd_g_lose_tantrum01 = {
		name = 'gsbd_g_lose_tantrum01'
		path = $lose_anim_path
	}
	gsbd_s_lose_angryatcrowd01 = {
		name = 'gsbd_s_lose_angryatcrowd01'
		path = $lose_anim_path
	}
	gsbd_s_lose_tantrum01 = {
		name = 'gsbd_s_lose_tantrum01'
		path = $lose_anim_path
	}
	guit_zakk_losel_1 = {
		name = 'guit_Zakk_LoseL_1'
		path = $lose_anim_path
	}
	guit_zakk_loser_2 = {
		name = 'guit_Zakk_LoseR_2'
		path = $lose_anim_path
	}
	g_jeff_lose_depressed = {
		name = 'g_jeff_lose_depressed'
		path = $lose_anim_path
	}
	g_jeff_lose_fearful = {
		name = 'g_jeff_lose_fearful'
		path = $lose_anim_path
	}
	g_jeff_lose_kickinstrument = {
		name = 'g_jeff_lose_kickinstrument'
		path = $lose_anim_path
	}
	g_jeff_lose_pretentious = {
		name = 'g_jeff_lose_pretentious'
		path = $lose_anim_path
	}
	g_jeff_lose_smash = {
		name = 'g_jeff_lose_smash'
		path = $lose_anim_path
	}
	g_jeff_lose_walkoff = {
		name = 'g_jeff_lose_walkoff'
		path = $lose_anim_path
	}
	g_jimi_loss_01 = {
		name = 'g_jimi_loss_01'
		path = $lose_anim_path
	}
	g_jimi_loss_drum = {
		name = 'g_jimi_loss_drum'
		path = $lose_anim_path
	}
	g_jimi_loss_left = {
		name = 'g_jimi_loss_left'
		path = $lose_anim_path
	}
	G_Kris_Lose_AngryAtCrowd = {
		name = 'G_Kris_Lose_AngryAtCrowd'
		path = $lose_anim_path
	}
	G_Kris_Lose_Depressed = {
		name = 'G_Kris_Lose_Depressed'
		path = $lose_anim_path
	}
	G_Kris_Lose_PointBass = {
		name = 'G_Kris_Lose_PointBass'
		path = $lose_anim_path
	}
	G_Kris_Lose_PointDrum = {
		name = 'G_Kris_Lose_PointDrum'
		path = $lose_anim_path
	}
	G_Kris_Lose_PointGuit = {
		name = 'G_Kris_Lose_PointGuit'
		path = $lose_anim_path
	}
	G_Kris_Lose_PointSing = {
		name = 'G_Kris_Lose_PointSing'
		path = $lose_anim_path
	}
	g_kris_lose_pretentious = {
		name = 'g_kris_lose_pretentious'
		path = $lose_anim_path
	}
	g_kris_lose_tantrum = {
		name = 'g_kris_lose_tantrum'
		path = $lose_anim_path
	}
	g_ted_lose_01 = {
		name = 'g_ted_lose_01'
		path = $lose_anim_path
	}
	g_ted_lose_pointbass = {
		name = 'g_ted_lose_pointbass'
		path = $lose_anim_path
	}
	g_ted_lose_pointdrum = {
		name = 'g_ted_lose_pointdrum'
		path = $lose_anim_path
	}
	g_ted_lose_pointsinger = {
		name = 'g_ted_lose_pointsinger'
		path = $lose_anim_path
	}
	s_billy_lose_boss_01 = {
		name = 's_billy_lose_boss_01'
		path = $lose_anim_path
	}
	s_billy_lose_boss_02 = {
		name = 's_billy_lose_boss_02'
		path = $lose_anim_path
	}
	S_Billy_Lose_Boss_GuitSmash = {
		name = 's_billy_lose_boss_guitsmash'
		path = $lose_anim_path
	}
	s_haley_lose_01 = {
		name = 's_haley_lose_01'
		path = $lose_anim_path
	}
	s_haley_lose_pointbass = {
		name = 's_haley_lose_pointbass'
		path = $lose_anim_path
	}
	s_haley_lose_pointdrum = {
		name = 's_haley_lose_pointdrum'
		path = $lose_anim_path
	}
	s_haley_lose_pointguitar = {
		name = 's_haley_lose_pointguitar'
		path = $lose_anim_path
	}
	s_jeff_lose_depressed = {
		name = 's_jeff_lose_depressed'
		path = $lose_anim_path
	}
	s_jeff_lose_fearful = {
		name = 's_jeff_lose_fearful'
		path = $lose_anim_path
	}
	s_jeff_lose_kickinstrument = {
		name = 's_jeff_lose_kickinstrument'
		path = $lose_anim_path
	}
	s_jeff_lose_pretentious = {
		name = 's_jeff_lose_pretentious'
		path = $lose_anim_path
	}
	s_jeff_lose_smash = {
		name = 's_jeff_lose_smash'
		path = $lose_anim_path
	}
	s_jeff_lose_walkoff = {
		name = 's_jeff_lose_walkoff'
		path = $lose_anim_path
	}
	S_Kris_Lose_AngryAtCrowd = {
		name = 'S_Kris_Lose_AngryAtCrowd'
		path = $lose_anim_path
	}
	S_Kris_Lose_Depressed = {
		name = 'S_Kris_Lose_Depressed'
		path = $lose_anim_path
	}
	S_Kris_Lose_PointBass = {
		name = 'S_Kris_Lose_PointBass'
		path = $lose_anim_path
	}
	S_Kris_Lose_PointDrum = {
		name = 'S_Kris_Lose_PointDrum'
		path = $lose_anim_path
	}
	S_Kris_Lose_PointGuit = {
		name = 'S_Kris_Lose_PointGuit'
		path = $lose_anim_path
	}
	S_Kris_Lose_PointSing = {
		name = 'S_Kris_Lose_PointSing'
		path = $lose_anim_path
	}
	s_kris_lose_pretentious = {
		name = 's_kris_lose_pretentious'
		path = $lose_anim_path
	}
	s_kris_lose_tantrum = {
		name = 's_kris_lose_tantrum'
		path = $lose_anim_path
	}
	s_ozzy_lose = {
		name = 's_ozzy_lose'
		path = $lose_anim_path
	}
	s_ozzy_lose_bucket = {
		name = 's_ozzy_lose_bucket'
		path = $lose_anim_path
	}
	s_ozzy_lose_pointbass = {
		name = 's_ozzy_lose_pointbass'
		path = $lose_anim_path
	}
	s_ozzy_lose_pointdrum = {
		name = 's_ozzy_lose_pointdrum'
		path = $lose_anim_path
	}
	s_ozzy_lose_pointguitar = {
		name = 's_ozzy_lose_pointguitar'
		path = $lose_anim_path
	}
	s_sting_lose_01 = {
		name = 's_sting_lose_01'
		path = $lose_anim_path
	}
	s_sting_lose_pointbass = {
		name = 's_sting_lose_pointbass'
		path = $lose_anim_path
	}
	s_sting_lose_pointdrum = {
		name = 's_sting_lose_pointdrum'
		path = $lose_anim_path
	}
	s_sting_lose_pointguitar = {
		name = 's_sting_lose_pointguitar'
		path = $lose_anim_path
	}
}
Anim_StarPower_Pakman_Links = {
	group_starpower_01 = {
		name = 'group_starpower_01'
		path = $starpower_anim_path
	}
	group_starpower_02 = {
		name = 'group_starpower_02'
		path = $starpower_anim_path
	}
}

script guitar_create_character_maps 
	mpm_new_map \{heap = heap_musician1
		links = Highway_Pakman_Links
		folder = 'pak/models/highway/'}
	mpm_new_map \{heap = heap_musician2
		links = Highway_Pakman_Links
		folder = 'pak/models/highway/'}
	mpm_new_map \{heap = heap_musician3
		links = Highway_Pakman_Links
		folder = 'pak/models/highway/'}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Perm_Pakman_Links
		folder = 'pak/anims/'
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Loop_GUIT_SINGER_Pakman_Links
		folder = 'pak/anims/'
		index = 0
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Loop_GUIT_SINGER_Pakman_Links
		folder = 'pak/anims/'
		index = 1
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Loop_GUIT_SINGER_Pakman_Links
		folder = 'pak/anims/'
		index = 2
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Loop_DRUM_Pakman_Links
		folder = 'pak/anims/'
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 0
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 1
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 2
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 3
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 4
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 5
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 6
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Intro_Win_Pakman_Links
		folder = 'pak/anims/'
		index = 7
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Lose_Pakman_Links
		folder = 'pak/anims/'
		index = 0
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Lose_Pakman_Links
		folder = 'pak/anims/'
		index = 1
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Lose_Pakman_Links
		folder = 'pak/anims/'
		index = 2
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_Lose_Pakman_Links
		folder = 'pak/anims/'
		index = 3
		no_vram}
	mpm_new_map \{heap = heap_musician_anim
		links = Anim_StarPower_Pakman_Links
		folder = 'pak/anims/'
		index = 0
		no_vram}
endscript

script guitar_force_unload_anim_paks 
	mpm_flush_all_paks_eachlink \{checksum = Anim_Loop_DRUM_Pakman_Links}
	mpm_flush_all_paks_eachlink \{checksum = Anim_Loop_GUIT_SINGER_Pakman_Links}
	if NOT GotParam \{not_wli}
		mpm_flush_all_paks_eachlink \{checksum = Anim_Intro_Win_Pakman_Links}
		mpm_flush_all_paks_eachlink \{checksum = Anim_Lose_Pakman_Links}
		mpm_flush_all_paks_eachlink \{checksum = Anim_StarPower_Pakman_Links}
	endif
endscript

script guitar_destroy_character_maps 
	mpm_free_all_maps
endscript
